...
import { Clock, Banknote, ShoppingCart, CalendarDays, Bike, ShowerHead, Sofa, Laptop, Droplets, Accessibility, Calendar } from "lucide-react";
    ...
    <>
      ...
      <div className="container mx-auto px-4 py-16 space-y-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
          ...
          {/* Booking card — replaces broken iframe */}
          <div className="rounded-2xl border-2 border-brand/20 bg-gradient-to-br from-brand/5 to-transparent p-8 flex flex-col items-center justify-center text-center gap-5">
            <div className="w-16 h-16 rounded-full bg-brand/10 flex items-center justify-center">
              <Calendar className="w-8 h-8 text-brand" />
            </div>
            <div>
              <h3 className="font-buzz text-2xl mb-2">Réservez en ligne</h3>
              <p className="text-gray-500 text-sm max-w-xs">
                Créneaux disponibles 5 jours à l&apos;avance. Réservez directement sur Playtomic en quelques secondes.
              </p>
            </div>
            <a
              href="https://playtomic.com/clubs/padel-15"
              target="_blank"
              rel="noopener noreferrer"
              className="bg-brand hover:bg-brand-dark text-white font-semibold px-10 py-4 rounded-xl text-lg transition-colors shadow-lg shadow-brand/20 w-full sm:w-auto"
            >
              Réserver sur Playtomic
            </a>
            <p className="text-gray-400 text-xs">Aucun compte requis • Confirmation immédiate</p>
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-2xl p-8">
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-gray-700">
            {[
              { icon: <Bike className="w-4 h-4 text-brand" />, text: "Garage à vélo sécurisé" },
              { icon: <ShowerHead className="w-4 h-4 text-brand" />, text: "Vestiaires, douches et toilettes" },
              { icon: <Sofa className="w-4 h-4 text-brand" />, text: "Espace détente intérieur" },
              { icon: <Laptop className="w-4 h-4 text-brand" />, text: "Coin coworking" },
              { icon: <Droplets className="w-4 h-4 text-brand" />, text: "Fontaine à eau en libre-service" },
              { icon: <Accessibility className="w-4 h-4 text-brand" />, text: "Accessibilité PMR totale" },
            ].map((item) => (
